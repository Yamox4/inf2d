...
use inf3d_camera::{CameraRig, IsoCamera};
use inf3d_core::{
...
use inf3d_worldgen::{Terrain, VoxelOverrides, WorldGen};
...
fn apply_world_load(
    ...
    match kind {
        LoadKind::New => {
            world_gen.set_flat(true); // flip BEFORE reading stand_pos below
            overrides.clear_all();
            testmap::stamp_test_map(&overrides);
            // Reset the player onto the flat spawn (capsule center = feet + offset,
            // mirroring `spawn_player`).
    ...
    info!("inf3d_menu: world ready (flat = {})", world_gen.is_flat());
}
...
fn do_save(
    ...
    let (yaw, zoom) = rig_q.single().map(|r| (r.yaw(), r.zoom())).unwrap_or((0.0, 44.0));
    let game = SaveGame {
    ...
    for (interaction, button) in &buttons {
        ...
        match button.0 {
            MenuAction::StartGame => {
                pending_load.0 = Some(LoadKind::New);
...
fn spawn_main(commands: &mut Commands) {
    screen!(commands, "inf3d", panel => {
        ...
        menu_button!(panel, theme::button_node(), "Start Game", 22.0, MenuAction::StartGame);
