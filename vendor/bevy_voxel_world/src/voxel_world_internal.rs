    ...
    C: VoxelWorldConfig,
{
    ...
    pub fn spawn_chunks(
        ...
        while let Some(chunk_position) = chunks_deque.pop_front() {
            ...
            // Local fork: clamp the streamed region to a vertical chunk-Y slab
            // (heightfield worlds occupy a shallow band; the rest of the sphere
            // is empty air/underground). No-op when `chunk_y_bounds()` is `None`.
            if let Some((min_y, max_y)) = configuration.chunk_y_bounds() {
    ...
    pub fn retire_chunks(
        ...
        let chunks_to_remove = {
            ...
            for (chunk, view_visibility) in all_chunks.iter() {
                ...
                // Local fork: retire chunks outside the vertical Y slab so the
                // clamp stays consistent with spawning. No-op when `None`.
                let y_out_of_bounds = configuration
                    .chunk_y_bounds()
                    .is_some_and(|(min_y, max_y)| {
                        chunk.position.y < min_y || chunk.position.y > max_y
                    });
